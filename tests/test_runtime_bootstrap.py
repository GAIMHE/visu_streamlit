...
def test_bootstrap_runtime_assets_preserves_explicit_empty_required_paths(monkeypatch) -> None:
    monkeypatch.setattr(runtime_bootstrap, "_secrets_mapping", lambda: None)
    monkeypatch.setattr(
        runtime_bootstrap,
        "_publish_runtime_secrets_to_env",
    ...
    monkeypatch.setattr(runtime_bootstrap, "load_hf_repo_config", lambda **kwargs: config)
