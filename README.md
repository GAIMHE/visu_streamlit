...
This page keeps the structural ZPDES lane layout, then compares event-based progression cohorts for each activity.  
...
- Hover-based all-work-mode first-attempt summaries plus before/after/in-activity cohort summaries (cohorts restricted to `zpdes` or `playlist` exercise-first-attempt events only)  
